...
{
  Timestamp beforeInit;
  for (int i = 0; i < numPipes; ++i)
  {
    Channel& channel = *g_channels[i];
    channel.SetReadCallback(std::bind(readCallback, _1, channel.Getfd(), i));
  ...
  g_reads = 0;
  g_writes = numWrites;
  Timestamp beforeLoop;
  g_loop->Loop();

  Timestamp end;

  int iterTime = static_cast<int>(end.ToUsecs() - beforeInit.ToUsecs());
  int loopTime = static_cast<int>(end.ToUsecs() - beforeLoop.ToUsecs());
